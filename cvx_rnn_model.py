...
def generate_chevron_rnn(start_dates, end_dates, seed):
    seed_value = seed
    ...
    for i in range(len(data_scaled) - sequence_length):
        ...
        close_price_today = data_scaled[i + sequence_length][3]  # CVX_Close index after scaling
        close_price_prev = data_scaled[i + sequence_length - 1][3]  # Previous day's CVX_Close
    ...
    plt.xlabel('Bins')
    plt.ylabel('Count')
    plt.title(f'Distribution of Actual vs Predicted Percent Changes (CVX RNN - Seed {seed_value}')
    plt.legend()
    plt.show()
    ...
    if y_pred.size == 0:
        ...
        plt.title(f'Expected vs Actual Percent Change in Closing Prices (CVX RNN - Seed {seed_value})')
        plt.xlabel('Date')
